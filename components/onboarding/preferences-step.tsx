...
export function PreferencesStep({ data, onChange }: PreferencesStepProps) {
  const handleServiceChange = (service: string, checked: boolean) => {
    const services = checked ? [...data.services, service] : data.services.filter((s: string) => s !== service)
  ...
  const handleRegionChange = (region: string, checked: boolean) => {
    const regions = checked ? [...data.regions, region] : data.regions.filter((r: string) => r !== region)
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-gray-600 mb-4">Select all services you provide</p>
      ...
      <div>
        ...
        <p className="text-sm text-gray-600 mb-4">Select up to 10 regions you service</p>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {regionOptions.map((region) => (
            <div key={region} className="flex items-center space-x-2">
              <Checkbox
                ...
                disabled={data.regions.length >= 10 && !data.regions.includes(region)}
              />
      ...
      <div>
        ...
        <p className="text-sm text-gray-600 mb-4">Rate your preference for different types of work (1-10 scale)</p>
          ...
          {Object.entries(data.jobPreferences).map(([type, value]) => (
            <div key={type} className="space-y-2">
              ...
              <Slider
                ...
                onValueChange={(newValue) => handleJobPreferenceChange(type, newValue)}
                max={10}
                min={1}
                step={1}
                className="w-full"
